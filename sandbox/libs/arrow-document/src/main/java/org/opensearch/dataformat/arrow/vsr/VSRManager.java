...
/**
 * Top-level orchestrator for the Arrow batching -> data file generation pipeline.
 *
 * <p>Combines {@link VSRPool} (Arrow batch management) with {@link NativeFormatWriter}
 * (the format's native writer) to provide a single entry point for document ingestion.
 * Handles the complete flow:
 * <ol>
 *   <li>{@link #addDocument(ArrowDocumentInput)} — transfers document fields into the active
 ...
 *       finalizes the data file, and returns file metadata.</li>
 * </ol>
 *
...
public class VSRManager implements AutoCloseable {
    ...
    private final ArrowIngestStats stats;
    private volatile Future<?> pendingWrite;
    private final NativeFormatWriter writer;
     ...
     *
     * @param fileName output data file path
    ...
    public VSRManager(
        ...
        this.vsrRotationThread = runAsync ? rotationThreadPoolName : ThreadPool.Names.SAME;
     ...
     * <p>
     * Single-value semantics are enforced at the {@link ArrowDocumentInput} layer:
     ...
     */
    public void addDocument(ArrowDocumentInput doc) throws IOException {
        ...
        // Re-check the rowId invariant so a single-format path is protected too.
        ...
        try {
            for (FieldValuePair pair : doc.getFinalInput()) {
                ...
                ArrowField arrowField = ArrowFieldRegistry.getArrowField(fieldType.typeName());
                if (arrowField == null) {
                    // Defense-in-depth: schema reconciliation is supposed to happen in
                    // the format writer's updateMappingVersion before any addDocument with a new
                    ...
                    throw new MismatchedInputException(
                        "No ArrowField mapping for field [" + fieldType.name() + "] of type [" + fieldType.typeName() + "]"
                    );
                }
                ...
                arrowField.createField(fieldType, activeVSR, pair.getValue());
                writtenFields++;
            }
     ...
     * {@code writtenFields} fields of {@link ArrowDocumentInput#getFinalInput()} (which is written in
     ...
     */
    private void scrubPartialRow(ArrowDocumentInput doc, ManagedVSR activeVSR, int rowIndex, int writtenFields, boolean rowIdWritten) {
     ...
     * whose successful write already allocated them, so no growth occurs. Every vector type the
     ...
     * <p>
     * Called from the format writer's {@code updateMappingVersion}
    ...
    /**
     * Flushes the current VSR data to a data file via the format's native writer.
     *
     * @return metadata about the written data file, or null if no data to flush
     */
    public FormatFileMetadata flush() throws IOException {
        ...
        FormatFileMetadata metadata = writer.flush();
    ...
    /**
     * Syncs the data file to disk. Must be called after {@link #flush()}.
     */
    ...
    private void maybeInitializeWriter(ManagedVSR vsr) throws IOException {
        if (writer.isInitialized() == false) {
            ...
            FormatSortConfig sortConfig = new FormatSortConfig(indexSettings);
