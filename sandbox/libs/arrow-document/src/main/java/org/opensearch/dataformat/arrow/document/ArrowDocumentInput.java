...
/**
 * Document input that collects mapped fields for an Arrow-backed data format.
 *
 ...
 * to Arrow vectors by {@link org.opensearch.dataformat.arrow.vsr.VSRManager#addDocument(ArrowDocumentInput)}.
 *
 ...
 */
public class ArrowDocumentInput implements DocumentInput<List<FieldValuePair>> {

    private static final Logger logger = LogManager.getLogger(ArrowDocumentInput.class);

    /**
     * The format this input feeds. Only used to look up which capabilities a field was assigned to
     * this format, which is what makes the class reusable across formats.
     */
    ...
    /**
     * @param dataFormat the format consuming this input; fields not assigned to it are skipped
     */
    public ArrowDocumentInput(DataFormat dataFormat) {
    ...
    public void addField(MappedFieldType fieldType, Object value) {
        ...
        Set<FieldTypeCapabilities.Capability> capabilities = fieldType.getCapabilityMap()
            .getOrDefault(dataFormat, Set.of());
    ...
    public List<FieldValuePair> getFinalInput() {
        if (!isClosed) {
            ...
            // assertions for the primary format
            // TODO: once the primary format may also run as a secondary, this assertion would change
